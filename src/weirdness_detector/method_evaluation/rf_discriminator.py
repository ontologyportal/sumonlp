...
Train a Random Forest coherence discriminator on dev.csv and apply to test.csv.

Same I/O + threshold selection as svm_discriminator.py.

Model:
  RandomForestClassifier + predict_proba

Notes:
- RF does not need feature scaling.
- RF can overfit; keep depth/min_samples_* conservative.

Fallback behavior (IMPORTANT):
- If no threshold satisfies the precision constraint (with min_accepted), we choose the best achievable:
    - If --use-lcb: pick threshold (with accepted>=min_accepted) maximizing Wilson LCB.
    - Else: pick threshold (with accepted>=min_accepted) maximizing raw precision.
- If even accepted>=min_accepted is impossible, fall back to accept-none (t=1.0).
"""
...
def choose_threshold(
    ...
    """
    Primary selection:
      - Among thresholds satisfying (accepted>=min_accepted) and precision constraint,
        choose the one maximizing coherent_kept_rate.

    Fallback (if none satisfy precision constraint):
      - If use_lcb: choose threshold (accepted>=min_accepted) maximizing Wilson LCB.
      - Else: choose threshold (accepted>=min_accepted) maximizing raw precision.
      - If no thresholds satisfy accepted>=min_accepted, fall back to accept-none (t=1.0).
    """
    best_t = 1.0
    best_ckr = -1.0
...
def eval_on_test(path: Path) -> None:
    ...
    print("\n=== Random Forest Discriminator Evaluation ===")
    print(f"File: {path}")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("dev_csv")
    p.add_argument("test_csv")
    p.add_argument("--out", default="test_scored_rf.csv")
    ...
    p.add_argument("--min-samples-split", type=int, default=10)
